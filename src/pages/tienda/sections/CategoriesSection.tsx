...
export const CategoriesSection: React.FC<CategoriesSectionProps> = ({ activeCategory, setActiveCategory }) => {
  const categorias = [
    { nombre: "Todos", img: "https://veterinariadelsol.com/wp-content/uploads/2023/09/lista4-scaled.jpg" },
  ...
  return (
    <section>
      ...
      <div className="flex overflow-x-auto pb-4 gap-4 sm:gap-6 custom-scrollbar">
        {categorias.map((cat, index) => {
          ...
          return (
            <div 
              ...
              className={`flex flex-col items-center gap-3 cursor-pointer group min-w-[80px] sm:min-w-[100px] transition-transform ${isActive ? 'scale-105' : 'hover:scale-105'}`}
            >
              <div className={`w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden border-4 transition-all p-1 shadow-sm ${isActive ? 'border-[#3B82F6] bg-blue-50 dark:bg-blue-900/20' : 'border-transparent bg-white dark:bg-[#1E293B]'}`}>
                <img src={cat.img} alt={cat.nombre} className="w-full h-full object-cover rounded-full" />
              </div>
              <span className={`text-sm font-bold transition-colors ${isActive ? 'text-[#3B82F6]' : 'text-[#64748B] dark:text-[#94A3B8] group-hover:text-[#0F172A] dark:group-hover:text-white'}`}>
                {cat.nombre}
              </span>
            </div>
          )
